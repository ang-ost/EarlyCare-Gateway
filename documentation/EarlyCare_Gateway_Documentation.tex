...
\subsection{Chain of Responsibility Pattern (Implemented)}
...
\begin{itemize}
    \item \textbf{ValidationHandler}: Validates incoming patient data for required fields
    \item \textbf{EnrichmentHandler}: Enriches data with calculated fields (e.g., age from birth date)
    \item \textbf{PrivacyHandler}: Applies privacy rules and data protection policies
    \item \textbf{TriageHandler}: Performs triage assessment based on clinical data
\end{itemize}

\begin{lstlisting}[language=Python, caption=Chain of Responsibility Implementation]
class DataProcessingPipeline:
    def __init__(self):
        ...
        self.validation.set_next(self.enrichment)\
            .set_next(self.privacy)\
            .set_next(self.triage)
...
\begin{lstlisting}[language=Python, caption=Pipeline Usage in Patient Creation]
if data_pipeline:
    patient_dict = {
...
\subsection{Strategy Pattern (Implemented)}
...
\begin{itemize}
    \item \textbf{GeminiStrategy}: Google Gemini AI (active)
    \item \textbf{OpenAIStrategy}: OpenAI GPT (placeholder for future implementation)
    \item \textbf{ClaudeStrategy}: Anthropic Claude (placeholder for future implementation)
\end{itemize}

\begin{lstlisting}[language=Python, caption=Strategy Pattern Implementation]
...
\subsubsection{Runtime Model Selection}

The system initializes with Gemini as the default strategy and allows switching models at runtime:

\begin{lstlisting}[language=Python, caption=Strategy Usage in Diagnosis Generation]
...
\subsection{Observer Pattern (Implemented)}

The Observer pattern provides production monitoring through event logging, metrics tracking, and alerting capabilities.

\subsubsection{Observer Types}

\begin{itemize}
    \item \textbf{AuditLogObserver}: Logs all system events to audit trail database
    \item \textbf{MetricsObserver}: Tracks system metrics (event counts, types, timestamps)
    \item \textbf{AlertObserver}: Sends alerts when error thresholds are exceeded
\end{itemize}

\begin{lstlisting}[language=Python, caption=Observer Pattern Implementation]
...
\subsubsection{Event Monitoring}

Events are logged throughout the application lifecycle:

\begin{lstlisting}[language=Python, caption=Observer Usage for Event Logging]
...
\subsubsection{Metrics Retrieval}

Real-time metrics are accessible via API endpoint:

\begin{lstlisting}[language=Python, caption=Metrics API Endpoint]
...
\subsection{Facade Pattern (Implemented)}
...
\subsubsection{Integrated Systems}

\begin{itemize}
    \item \textbf{HL7Interface}: Health Level 7 messaging (ADT, ORM messages)
    \item \textbf{FHIRInterface}: FHIR REST API (Patient, Observation resources)
    \item \textbf{PACSInterface}: Picture Archiving and Communication System
    \item \textbf{LISInterface}: Laboratory Information System
\end{itemize}

\begin{lstlisting}[language=Python, caption=Facade Pattern Implementation]
class ClinicalSystemsFacade:
    ...
    def submit_clinical_record(self, record_data):
        results = {}
        if 'vital_signs' in record_data:
            results['fhir_observation'] = self.fhir.create_observation_resource(
                record_data['vital_signs']
            )
...
\subsubsection{Simplified Integration}

The facade simplifies complex multi-system operations:

\begin{lstlisting}[language=Python, caption=Facade Usage in Patient Registration]
if clinical_facade:
...
\toprule
\textbf{Pattern} & \textbf{Status} & \textbf{Implementation Details} \\
\midrule
...
\begin{itemize}
    ...
    \item \textbf{Extensibility}: Easy addition of new AI models, handlers, and observers
    \item \textbf{Monitoring}: Real-time system metrics and event tracking
    \item \textbf{Integration}: Simplified interface for complex external systems
    \item \textbf{Quality}: Consistent data processing and validation pipeline
    \item \textbf{Security}: Centralized authentication and audit logging
\end{itemize}
...
\subsection{Doctor ID Generation Algorithm}
...
\begin{itemize}
    \item \textbf{Format}: 6-character alphanumeric code (e.g., MR7X9Z, AB4K2L)
    \item \textbf{Structure}: First letter of first name + First letter of last name + 4 random characters
    \item \textbf{Character Set}: Uppercase letters (A-Z) and digits (0-9)
    \item \textbf{Randomness}: Uses Python's \texttt{secrets} module for cryptographically strong random generation
    \item \textbf{Uniqueness}: System checks for duplicates before assignment
\end{itemize}

\textbf{Example}: A doctor named "Mario Rossi" would receive an ID like "MR7X9Z" where:
\begin{itemize}
    \item M = First letter of "Mario"
    \item R = First letter of "Rossi"
    \item 7X9Z = Randomly generated secure suffix
\end{itemize}

\subsection{Admin-Controlled Registration}

To prevent unauthorized access and fraudulent doctor ID creation, the registration functionality is restricted:

\begin{itemize}
    \item \textbf{Production Mode}: Only authenticated administrators can create new doctor accounts
    \item \textbf{Regular Users}: See only the login form requiring existing doctor ID and password
    \item \textbf{Admin Users}: Access both login and registration forms to create accounts for legitimate medical professionals
    \item \textbf{ID Distribution}: Administrators securely share generated IDs with authorized doctors through verified channels
\end{itemize}

This two-tier access control ensures that:
\begin{enumerate}
    \item Non-medical personnel cannot obtain fake doctor IDs
    \item All doctor accounts are verified and authorized by administrators
    \item The system maintains a trusted registry of healthcare professionals
    \item Accountability is maintained through controlled account creation
\end{enumerate}

\subsection{Authentication Features}

\begin{itemize}
    \item SHA-256 password hashing for secure credential storage
    \item Session management with secure, HTTP-only cookies
    \item 7-day session lifetime with automatic expiration
    \item Support for doctor specialization and affiliated hospital tracking
    \item Login attempt monitoring and rate limiting
\end{itemize}
...
\subsection{Access Control and Registration Security}

The system implements a critical security measure to prevent unauthorized account creation:

\begin{itemize}
    \item \textbf{Admin-Only Registration}: Doctor account creation is restricted to authenticated administrators
    \item \textbf{Public Interface}: Regular users only see the login form, preventing self-registration
    \item \textbf{Verified Accounts}: All doctor accounts must be created by trusted administrators
    \item \textbf{ID Security}: The cryptographically generated doctor IDs cannot be guessed or forged
\end{itemize}

This approach prevents:
\begin{itemize}
    \item Unauthorized individuals from creating fake doctor accounts
    \item Self-registration by non-medical personnel
    \item Identity fraud and impersonation
    \item Uncontrolled access to sensitive patient data
\end{itemize}
\pagebreak
...
\subsubsection{Management Dashboard Features}

\begin{itemize}
    \item \textbf{User Data Management}: View, edit, and manage all registered doctor accounts
    \item \textbf{Role Assignment}: Dynamically assign or modify user roles (Doctor, Admin)
    \item \textbf{Testing Environment}: Secure environment for application testing and validation
    \item \textbf{Account Status Control}: Activate, deactivate, or delete user accounts
\end{itemize}

\subsubsection{Role Management Capabilities}

The management system enables administrators to:

\begin{enumerate}
    \item \textbf{Promote to Admin}: Elevate trusted doctors to administrator status for account management duties
    \item \textbf{Revoke Admin Rights}: Demote administrators back to regular doctor status when necessary
    \item \textbf{Deactivate Accounts}: Suspend access for retired doctors or those on leave
    \item \textbf{Remove Accounts}: Permanently delete accounts for struck-off doctors or terminated personnel
    \item \textbf{Audit Trail}: Track all role changes and account modifications for compliance
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Retirement}: Deactivating accounts for doctors who have retired from practice
    \item \textbf{Disciplinary Actions}: Removing access for doctors who have been struck off the medical register
    \item \textbf{Temporary Leave}: Suspending accounts during sabbaticals or extended leave
    \item \textbf{Admin Rotation}: Managing administrative privileges among trusted staff
    \item \textbf{Emergency Access}: Quickly granting or revoking access in critical situations
\end{itemize}

This centralized management approach ensures:
\begin{itemize}
    \item Efficient user lifecycle management
    \item Rapid response to personnel changes
    \item Maintained system security and integrity
    \item Compliance with healthcare regulatory requirements
    \item Clear accountability through comprehensive audit logging
\end{itemize}

\subsection{Doctor Registration}
\begin{itemize}
    \item Unique mnemonic ID generation using \texttt{secrets} module
...
\begin{itemize}
    \item \textbf{Automatic HTTPS}: Free SSL certificates with automatic renewal
    \item \textbf{Git Integration}: Automatic deployments triggered by repository commits
    \item \textbf{Zero-Downtime Deploys}: Rolling updates without service interruption
    \item \textbf{Managed Infrastructure}: No server management required
    \item \textbf{Environment Variables}: Secure configuration management through web dashboard
    \item \textbf{Custom Domains}: Support for custom domain mapping
    \item \textbf{Health Checks}: Automatic service monitoring and restart
\end{itemize}
...
\subsubsection{Backend Service (Web Service)}

\begin{itemize}
    \item \textbf{Type}: Web Service
    \item \textbf{Runtime}: Python 3.8+
    \item \textbf{Build Command}: \texttt{pip install -r backend/requirements.txt}
    \item \textbf{Start Command}: \texttt{python backend/webapp/app.py}
    \item \textbf{Port}: Auto-detected from Flask application
    \item \textbf{Health Check Path}: \texttt{/api/health} (recommended)
\end{itemize}

\subsubsection{Frontend Service (Static Site)}

\begin{itemize}
    \item \textbf{Type}: Static Site
    \item \textbf{Build Command}: \texttt{cd frontend \&\& npm install \&\& npm run build}
    \item \textbf{Publish Directory}: \texttt{frontend/dist}
    \item \textbf{Auto-Deploy}: Enabled on main branch commits
\end{itemize}

\subsection{Deployment Steps}

\begin{enumerate}
    \item \textbf{Repository Connection}: Link GitHub/GitLab repository to Render
    \item \textbf{Service Creation}: Create separate services for backend and frontend
    \item \textbf{Environment Configuration}: Set required environment variables through Render dashboard
    \item \textbf{Build Configuration}: Define build and start commands for each service
    \item \textbf{Deploy}: Trigger initial deployment manually or via Git push
    \item \textbf{Domain Setup}: Configure custom domain or use provided \texttt{.onrender.com} subdomain
\end{enumerate}

\subsection{Environment Configuration on Render}
...
\begin{tabular}{ll}
\toprule
\textbf{Service} & \textbf{Required Variables} \\
\midrule
Backend & GEMINI\_API\_KEY \\
        & MONGODB\_CONNECTION\_STRING \\
        & FLASK\_SECRET\_KEY \\
        & FRONTEND\_URL \\
\midrule
Frontend & VITE\_API\_URL \\
\bottomrule
\end{tabular}
\caption{Render Environment Variables by Service}
\end{table}

\subsection{Production Considerations}

\begin{itemize}
    \item \textbf{Instance Type}: Select appropriate instance size based on expected traffic
    \item \textbf{Region Selection}: Choose region closest to primary user base for optimal latency
    \item \textbf{Scaling}: Configure auto-scaling policies for backend service
    \item \textbf{Monitoring}: Enable Render's built-in metrics and logging
    \item \textbf{Backup Strategy}: Ensure MongoDB Atlas backups are configured independently
\end{itemize}

\subsection{Continuous Deployment Workflow}

Render automatically deploys when changes are pushed to the configured branch:

\begin{enumerate}
    \item Developer pushes code to main branch
    \item Render detects commit via webhook
    \item Build process executes automatically
    \item Health checks verify successful deployment
    \item New version goes live with zero downtime
    \item Previous version remains available for instant rollback
\end{enumerate}
